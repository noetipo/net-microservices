...
{
    public class FindListProductsHandler : IRequestHandler<FindListProductsQuery, IEnumerable<ProductDto>>
    {
        private readonly IProductRepository _productRepository;

        public FindListProductsHandler(IProductRepository productRepository)
        {
            _productRepository = productRepository ?? throw new ArgumentNullException(nameof(productRepository));
        }

        public async Task<IEnumerable<ProductDto>> Handle(FindListProductsQuery request, CancellationToken cancellationToken)
        {
            var result = await _productRepository.FindListProductByProductCode(request.ProductCodeList ?? new List<string>());
